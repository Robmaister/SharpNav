		...
		private void SimplifyContour(List<RawVertex> points, List<SimplifiedVertex> simplified, float maxError, int maxEdgeLen, ContourBuildFlags buildFlags)
		{
			...
			for (int i = 0; i < simplified.Count;)
			{
				...
				if ((points[ci].RegionId & CONTOUR_REG_MASK) == 0 || (points[ci].RegionId & AREA_BORDER) != 0)
				{
					...
					while (ci != endi)
					{
						float deviation = MathHelper.DistanceFromPointToSegment(points[ci].X, points[ci].Z, ax, az, bx, bz);
			...
			if (maxEdgeLen > 0 && (buildFlags & (ContourBuildFlags.TessellateWallEdges | ContourBuildFlags.TessellateAreaEdges)) != 0)
			{
				for (int i = 0; i < simplified.Count;)
				{
					int ii = (i + 1) % simplified.Count;
					...
					//wall edges
					if ((buildFlags & ContourBuildFlags.TessellateWallEdges) != 0 && (points[ci].RegionId & CONTOUR_REG_MASK) == 0)
						tess = true;

					//edges between areas
					if ((buildFlags & ContourBuildFlags.TessellateAreaEdges) != 0 && (points[ci].RegionId & AREA_BORDER) != 0)
						tess = true;
			...
			for (int i = 0; i < simplified.Count; i++)
			{
				...
				//save new region id
				sv.RawVertexIndex = (points[ai].RegionId & (CONTOUR_REG_MASK | AREA_BORDER)) | (points[bi].RegionId & BORDER_VERTEX);
